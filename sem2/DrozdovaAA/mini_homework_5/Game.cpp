...
void Game::shuffleRemaining() {
    ...
    std::random_device rd;
    std::mt19937 g(rd());
    std::shuffle(remaining_symbols.begin(), remaining_symbols.end(), g); //перемешивание символов неразгаданных
...
void Game::update() {
    ...
    if (is_waiting_for_delay && delay_clock.getElapsedTime().asSeconds() >= 1.0f) {
...
void Game::render() {
    ...
    if (is_game_over) {
        ...
        ui_text.setPosition(window.getSize().x / 2.0f, window.getSize().y - 85.0f); //выравнивание
        window.draw(ui_text);
    }
    else {
        ...
        if (is_showing_shuffle_msg) {
            ...
            shuffle_text.setPosition(window.getSize().x / 2.0f, window.getSize().y - 45.0f);

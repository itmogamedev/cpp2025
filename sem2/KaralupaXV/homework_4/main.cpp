...
int main() {
	...
	std::generate(array.begin(), array.end(), [&distr, &gen]() { 
		return distr(gen);
	});
	...
	std::for_each(array.begin(), array.end(), [&output_file](int item) {
		output_file << item << " ";
	});
	output_file << "\n";
	...
	std::for_each(new_array.begin(), new_array.end(), [&output_file](int item) {
		output_file << item << " ";
	});
	output_file << "\n";

...
Game::Game() : window(sf::VideoMode({WINDOW_W, WINDOW_H}), "CPP game", sf::Style::Titlebar | sf::Style::Close), player({WINDOW_W / 2.f, WINDOW_H - 60.f}), ui(font) {
...
void Game::update(float dt) {
    ...
    spawnTimer += dt;
    spawnInterval = std::max(ASTEROID_SPAWN_MIN, ASTEROID_SPAWN_BASE - (float)(score / 8) * 0.08f);
    ...
    float astSpeed = ASTEROID_BASE_SPEED + (float)(score / 10) * ASTEROID_SPEED_STEP;
	...
	asteroids.erase(std::remove_if(asteroids.begin(), asteroids.end(), [](const Asteroid& b){ return !b.active; }), asteroids.end());
}

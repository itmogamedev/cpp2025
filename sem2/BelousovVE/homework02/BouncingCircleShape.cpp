...
BouncingCircleShape::BouncingCircleShape(float r, size_t pointCnt) : CircleShape(r, pointCnt) {
	speedX = rand() % NUMS::modX + NUMS::minSpeedX;
	speedY = rand() % NUMS::modY + NUMS::minSpeedY;

  this->setOrigin(sf::Vector2f(r, r));

	moveDir = sf::Vector2<int>(rand() % 2 == 0 ? 1 : -1, rand() % 2 == 0 ? 1 : -1);

  int x = r + 1;
  this->setPosition(sf::Vector2f(rand() % (SCREEN::width - x) + x, rand() % (SCREEN::height - x) + x));
...
void BouncingCircleShape::move() {
  ...
  this->setPosition(sf::Vector2f(pos.x + speedX * moveDir.x, pos.y + speedY * moveDir.y));
}

void BouncingCircleShape::updateDirectionOnCollisionWithWindow(sf::RenderWindow& wn) {
...
void BouncingCircleShape::draw(sf::RenderWindow& wn) {
	wn.draw(*this);
}

...
GameSession::GameSession()
    : mainWindow_(sf::VideoMode(WINDOW_WIDTH, WINDOW_HEIGHT),
      ...
      isActive_(true) {
  if (!uiFont_.loadFromFile("arial.ttf")) {
  ...
  gameoverDisplay_.setPosition(180.0f, 250.0f);
...
void GameSession::advance(float deltaTime) {
  ...
  for (auto& ball : balls_) {
    ball.move(deltaTime * VELOCITY_SCALE, WINDOW_WIDTH, WINDOW_HEIGHT);
  }
...
void GameSession::updateDisplayTexts() {
  ...
  timerDisplay_.setString(
      "Time: " + std::to_string(static_cast<int>(std::floor(remainingTime_))));
}

...
void Game::spawnTarget(int index) {
  ...
  targets[index].speed_x =
      static_cast<float>((std::rand() % 200 - 100) / 100.0) * BASE_SPEED;
  targets[index].speed_y =
      static_cast<float>((std::rand() % 200 - 100) / 100.0) * BASE_SPEED;
}
...
void Game::update(float delta_time) {
  ...
  for (int i = 0; i < TARGET_COUNT; ++i) {
    targets[i].shape.move(targets[i].speed_x * delta_time,
                          targets[i].speed_y * delta_time);
  ...
  time_text.setString("Time left: " +
                      std::to_string(static_cast<int>(std::ceil(time_left))));

...
const std::set<char> letters = { 'A', 'E', 'I', 'O', 'U', 'Y', 'a', 'e', 'i', 'o', 'u', 'y' };
...
int main() {
    ...
    if (file_in.is_open()) {
        std::string line;
        while (std::getline(file_in, line)) {lines.push_back(line);}

        file_in.close();
    } else { std::cerr << "Unable to open input.txt file" << std::endl; }

    auto f1 = [lines](int n) mutable { 
        for (int i = 0; i < lines.size(); ++i) {
            if (lines[i].size() < n) { lines.erase(lines.begin() + i--);}
        }
    ...
    auto f2 = [lines]() mutable {
        for (int i = 0; i < lines.size(); ++i) {
            for (int j = 0; j < lines[i].size(); ++j) {
                if (letters.find(lines[i][j]) != letters.end()) {
    ...
    auto f3 = [&lines](const std::string& word) {
        for (int i = 0; i < lines.size(); ++i) {
            if (lines[i].find(word) != std::string::npos) { return true; }
        }
    ...
    if (file_out.is_open()) {
        auto f_cout = [&file_out]<typename T>(T& s) mutable { file_out << s << '\n'; };
        ...
        file_out.close();
    } else { std::cerr << "Unable to open outpun.txt file" << std::endl; }
    return 0;
}

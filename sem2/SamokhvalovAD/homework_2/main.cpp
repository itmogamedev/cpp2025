...
void loop(sf::RenderWindow& window) {
  ...
  scoreText.setPosition(
      sf::Vector2f(WINDOW_WIDTH / 2 - scoreText.getGlobalBounds().size.x / 2,
                   SCORE_TEXT_OFFSET));
  // Timer text
  ...
  while (window.isOpen()) {
    timeLeft =
        GAME_DURATION - (int)clock.getElapsedTime().asSeconds() - increacedTime;
    if (timeLeft > 0) {
    ...
    timerText.setPosition(
        sf::Vector2f(WINDOW_WIDTH / 2 - timerText.getGlobalBounds().size.x / 2,
                     WINDOW_HEIGHT - TIMER_TEXT_OFFSET));

...
int main() {
    sf::RenderWindow window(sf::VideoMode(kWindowWidth, kWindowHeight), "Circle Clicker");
    ...
    for (int i = 0; i < 5; ++i) {
        sf::Vector2f pos = randomPosition(rng, kCircleRadius, kWindowWidth, kWindowHeight);
    ...
    while (window.isOpen()) {
            ...
            if (!game_over && event.type == sf::Event::MouseButtonPressed &&
                ...
                for (auto& circle : circles) {
                    if (circle.contains(mouse_pos)) {
                        ...
                        circle.shape.setPosition(randomPosition(rng, kCircleRadius, kWindowWidth, kWindowHeight));
        ...
        if (!game_over) {
            ...
            if (time_left <= 0.f) {
                ...
                final_score_text.setOrigin(final_score_text.getLocalBounds().width / 2, 0);
            }

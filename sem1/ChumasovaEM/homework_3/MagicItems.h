...
public:
    ...
    virtual std::variant<int, double, std::string> getSpecificParameter() const = 0;
...
std::vector<std::unique_ptr<MagicItem>> readShop(std::ifstream& file, std::string shopName);
shopStats calculateStats(std::string shopName, const std::vector<std::unique_ptr<MagicItem>>& items);

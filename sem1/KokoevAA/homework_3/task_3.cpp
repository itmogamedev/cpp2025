...
class magic_item {
 public:
  ...
  magic_item(const std::string& n, int p, double w)
...
void print_shop_stats(const shop_t& shop) {
  ...
  if (total_items > 0) {
    std::cout << "Средняя цена: " << sum_price / total_items << " золота"
              << std::endl;
    std::cout << "Средний вес: " << sum_weight / total_items << " кг"
              << std::endl;
  }
  ...
  if (count_potion > 0) {
    ...
    std::cout << "- Зелья: средняя длительность = " << avg_duration
              << std::endl;
  } else {
